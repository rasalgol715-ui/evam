<template>
  <div class="agent-wrapper">
    ...
    <div class="agent-bubble" @click="open = !open">
      <img src="/public/images/bot-avatar.png" alt="assistant" />
    </div>
    ...
    <Transition name="slide">
      <div v-if="open" class="agent-panel">
        ...
        <div ref="box" class="chat-box">
          <div v-for="(m, idx) in messages" :key="idx" :class="m.isBot ? 'bot' : 'user'">
            <p>{{ m.text }}</p>
        ...
        <div class="chat-bar">
          <input v-model="question" placeholder="Tape ta question…" @keydown.enter="send" />
...
const box = ref(null) // ← DÉCLARATION MANQUANTE

const systemPrompt = `Tu es "Prof-Eva", assistant pédagogique sénégalais, spécialisé en maths, physique, informatique du collège/lycée.
...
async function send() {
  ...
  try {
    ...
    messages.value.push({
      text: 'Désolé, le professeur est momentanément indisponible 😅 Contacte-nous sur WhatsApp.',
      link: 'https://wa.me/221775632268',
      linkText: 'WhatsApp',
    })
  }
  typing.value = false
...
.agent-bubble img {
  width: 32px; /* taille icône */
  height: 32px;
  object-fit: contain; /* évite déformation */
  filter: drop-shadow(0 2px 4px rgba(0, 0, 0, 0.2)); /* légère ombre */
}
.agent-panel {
  width: 340px;
  height: 480px;
